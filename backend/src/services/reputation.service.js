...
class ReputationService {
  ...
  static async recalculateReputation(userId) {
    try {
      ...
      // 1. Update Review Score (client reviews for this freelancer)
      ...
      // Reset breakdown before recomputing
      reputation.scores.reviews.breakdown = {
        5: 0,
        4: 0,
        3: 0,
        2: 0,
      ...
      if (reviews.length > 0) {
        ...
        // Update breakdown counts for 1-5 star ratings
        reviews.forEach((review) => {
          const stars = review.rating?.overall;
          if (stars && stars >= 1 && stars <= 5) {
            reputation.scores.reviews.breakdown[stars] =
              (reputation.scores.reviews.breakdown[stars] || 0) + 1;
          }
        });
      } else {
        reputation.scores.reviews.averageRating = 0;
        reputation.scores.reviews.totalReviews = 0;
        reputation.scores.reviews.score = 0;
      }

      // 2. Update Endorsement Score
      ...
      // 7. Auto-award achievements based on scores
      await this.awardAchievements(reputation);

      // 8. Calculate and save final scores
      await reputation.updateAllScores();

      // 9. Sync aggregate scores back to freelancer profile
      if (profile) {
        profile.localScore = reputation.localTrustScore;
        profile.globalScore = reputation.overallScore;
        profile.skillScore = reputation.skillTrustScore;
        await profile.save();
      }
