...
if (process.env.VERCEL !== '1') {
  ...
  autoApproveVerifications().catch(err => console.error('Error running auto-approve job on startup:', err));

  // Then run it every hour
  setInterval(() => {
    autoApproveVerifications().catch(err => console.error('Error in scheduled auto-approve job:', err));
  }, 60 * 60 * 1000); // 1 hour
