...
export const getFreelancerAnalytics = async (req, res, next) => {
  try {
    ...
    const analytics = {
      ...
      // Use actual completed contracts as the source of truth so
      // freelancer dashboards always match contract/job status.
      completedJobs: completedContracts.length,
      // If successRate is stored on the profile, keep using it; otherwise
      // derive it from the contract history.
      successRate: typeof profile.successRate === 'number' && profile.successRate >= 0
        ? profile.successRate
        : (totalContracts > 0
        ? parseFloat(((completedContracts.length / totalContracts) * 100).toFixed(2))
        : 0),
