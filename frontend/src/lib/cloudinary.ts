...
// Normalized upload result used by the frontend.
// width/height are only populated for image uploads.
...
/**
 * Upload a file via the backend (backend → Cloudinary).
 * For images, callers should still respect validateImageFile if they
 * want to enforce image-only uploads at the UI level.
 */
export const uploadToCloudinary = async (file: File, folder?: string): Promise<UploadResult> => {
  const raw: any = await api.uploadFile(file, folder);
  return {
    // Backend guarantees `url` is Cloudinary's secure_url (canonical public URL)
    url: raw.url,
