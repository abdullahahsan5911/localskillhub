...
export const UserHoverCard = ({ user, children }: UserHoverCardProps) => {
    ...
    const handleFollow = async (e: any) => {
        ...
        if (!currentUser) {
            const isNowFollowing = toggleGuestFollow(displayUser.id);
            setFollowing(isNowFollowing);
            setFollowerCount((prev) => {
                if (prev === undefined || prev === null) return isNowFollowing ? 1 : 0;
                return isNowFollowing ? prev + 1 : Math.max(prev - 1, 0);
            });
            return;
        }
                        ...
                        <Button
                            ...
                            onClick={(e) => {
                                ...
                                if (displayUser.role === "client") {
                                            navigate(`/clients/${displayUser.id}`);
                                        } else {
                                            navigate(`/freelancers/${displayUser.id}`);
                                        }
                            }}
                        >
                            View profile
