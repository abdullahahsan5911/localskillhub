...
const People = () => {
  ...
  const sortFreelancers = (list: PeopleFreelancer[], option: PeopleSortOption) => {
    ...
    const getInterestMatchScore = (freelancer: PeopleFreelancer) => {
      if (interestKeywords.length === 0) return 0;

      const haystack = [
        freelancer.title,
        ...(freelancer.skills || []).map((skill) => skill.name),
      ]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
    ...
    const getNewestSignal = (freelancer: PeopleFreelancer) => {
      ...
      if (profileCreatedAt || userCreatedAt) {
        return Math.max(profileCreatedAt, userCreatedAt);
      }
      ...
      return Math.max(profileFromId, userFromId);
    };
    ...
    sorted.sort((a, b) => {
      ...
      switch (option) {
        ...
        case "Most jobs":
          return jobsB - jobsA || createdAtB - createdAtA || ratingB - ratingA;
        case "Newest":
        ...
        default:
          ...
          return scoreB - scoreA || ratingB - ratingA || jobsB - jobsA || createdAtB - createdAtA;
      }
    });
  ...
  const getCoverImage = (freelancer: PeopleFreelancer) => {
    // Prefer banner image first
    if (freelancer.userId?.bannerImage) {
      return freelancer.userId.bannerImage;
    }
    // Fall back to portfolio image
            ...
            <div className="grid gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {freelancers.map((freelancer) => {
                ...
                return (
                  ...
                  >
                    <Link to={`/freelancers/${user._id}`} className="block">
                      <div className="relative h-32 sm:h-40 w-full bg-gray-100">
                        ...
                        <div className="absolute flex items-center justify-center -bottom-7 left-4 right-4">
                          <img
                            ...
                            alt={user.name}
                            className="h-16 w-16 sm:h-20 sm:w-20 rounded-full border-2 border-white object-cover shadow-md"
                          />
                        </div>
                      </div>
                      ...
                      <div className="px-4 pt-9 pb-4">
                        ...
                        <div className="flex flex-col items-center justify-between text-sm  text-gray-500">
                          ...
                          <div className="grid grid-cols-3 gap-2 w-full text-[10px] sm:text-sm text-gray-500 mb-4 px-2">
                            <div className="flex flex-col items-center border-r border-gray-100">
                              <span className="font-semibold text-gray-900 leading-tight">{formatStatsNumber(freelancer.completedJobs)}</span>
                              <span className="text-[10px] text-gray-400">jobs</span>
                            </div>
                            <div className="flex flex-col items-center border-r border-gray-100">
                              <span className="font-semibold text-gray-900 leading-tight">{formatStatsNumber(followersCount)}</span>
                              <span className="text-[10px] text-gray-400">followers</span>
                            </div>
                            <div className="flex flex-col items-center">
                              <span className="font-semibold text-gray-900 leading-tight">{formatStatsNumber(freelancer.profileViews)}</span>
