...
import {
  ...
  Globe, Users, Star, Clipboard, Coins,
  Currency, Paperclip, Trash2
} from "lucide-react";
...
const PostJob = () => {
  ...
  const isCompanyMode = scope === 'company' && user?.accountType === 'company';
  ...
  const [checkingCompanyProfile, setCheckingCompanyProfile] = useState(false);
  const [companyProfileMissing, setCompanyProfileMissing] = useState<string[]>([]);
  const [companyPostLocked, setCompanyPostLocked] = useState(false);
  ...
  useEffect(() => {
    ...
    const checkCompanyProfile = async () => {
      if (isLoading || !isAuthenticated || !user || user.role !== "client" || user.accountType !== "company") {
        if (!isMounted) return;
        setCompanyProfileMissing([]);
        setCompanyPostLocked(false);
        return;
      }

      setCheckingCompanyProfile(true);
      try {
        ...
        setCompanyPostLocked(missing.length > 0);
      } catch {
        if (!isMounted) return;
        setCompanyProfileMissing(["company profile"]);
        setCompanyPostLocked(true);
      } finally {
        if (isMounted) setCheckingCompanyProfile(false);
      }
    };
  ...
  const isCompanyPostLockedNow = isCompanyMode && !editJobId && companyPostLocked;
    ...
    const loadExistingJob = async () => {
      ...
      try {
        ...
        setForm({
          ...
          attachments: (job.attachments || []).map((file: any) => ({
  ...
  const handleUploadDocs = async (files: FileList | null) => {
    ...
    setError("");
    try {
      const uploadedDocs: JobForm["attachments"] = [];
      for (let i = 0; i < files.length; i += 1) {
        const file = files[i];
        const uploaded: any = await api.uploadFile(file, "job-docs");
      ...
      if (uploadedDocs.length > 0) {
        update("attachments", [...form.attachments, ...uploadedDocs]);
      }
    } catch (e: any) {
  ...
  const removeAttachment = (index: number) => {
    update(
      "attachments",
      form.attachments.filter((_, idx) => idx !== index),
    );
  };
  ...
  const handleSubmit = async () => {
    if (isCompanyPostLockedNow) {
      setError(`Complete company profile before posting: ${companyProfileMissing.join(", ")}`);
      return;
    }
    ...
    try {
      ...
      if (isCompanyMode) {
        navigate("/company-dashboard?tab=my-jobs");
      } else {
        navigate("/dashboard/client?tab=my-jobs");
      }
    } catch (e: any) {
        ...
        <div className=" w-full max-w-5xl mx-auto px-4">
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-gray-900">
              {editJobId ? 'Edit Job' : isCompanyMode ? 'Post a Company Job' : 'Post a Job'}
            </h1>
            <p className="text-gray-500 mt-1">
              {editJobId
                ? 'Update this job so it appears correctly on maps'
                : isCompanyMode
                  ? 'Create a new opening under your company profile'
                  : 'Find the perfect freelancer for your project'}
            </p>
            {!editJobId && (
              <p className="text-xs text-gray-700 mt-2">Your progress is automatically saved</p>
            )}
          </div>
          ...
          {isCompanyPostLockedNow && (
            <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
              Complete your company profile before posting jobs. Missing: {companyProfileMissing.join(", ")}.
              <Button
                type="button"
                variant="outline"
                size="sm"
          ...
          {checkingCompanyProfile && isCompanyMode && !editJobId && (
            <div className="mb-6 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
              Checking company profile requirements...
            </div>
          )}
              ...
              {STEP_TITLES.map((title, i) => {
                ...
                return (
                  <div key={title} className="flex items-center gap-2">
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all ${done ? "bg-green-500 text-white" : active ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-500"
                      }`}>
            ...
            {step === 1 && (
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    className="w-full px-4 py-3 border border-slate-400 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"
                ...
                <div>
                  ...
                  <textarea
                    rows={7}
                    className="w-full px-4 py-3 border border-slate-400 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none transition-all"
                ...
                <div>
                  <div className="mb-2 flex items-center justify-between gap-2">
                    <label className="block text-sm font-semibold text-gray-800">Project Documentation (optional)</label>
                    <input
                      ref={docsInputRef}
                      type="file"
                      multiple
                    ...
                    {CATEGORIES.slice(0, 12).map(cat => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center gap-2 p-3 rounded-xl border border-slate-300 text-sm font-medium transition-all ${selected
                              ? "border-blue-700 bg-blue-50 text-blue-700"
                              : "border-gray-200 bg-white text-gray-700 hover:border-gray-300"
                            }`}
                        >
                          <Icon className="w-4 h-4 flex-shrink-0"
                            style={{ color: selected ? undefined : cat.color }}
                          />
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      ...
                      <button
                        ...
                        className={`p-4 rounded-xl border border-slate-300 text-center transition-all ${form.experienceLevel === opt.value
              ...
              <div className="space-y-5">
                    ...
                    {[
                      ...
                      return (
                        <button
                          ...
                          className={`p-4 rounded-xl border flex items-center gap-3 transition-all ${form.budgetType === opt.value
                ...
                {form.budgetType === "fixed" ? (
                  <div>
                    ...
                    <label className="block text-lg font-semibold text-gray-800 mb-2">Budget Amount </label>
                    ...
                    <div className="relative">
                      ...
                      <input
                        type="number"
                        className="w-full pl-8 pr-4 py-3 border border-slate-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 [&::-webkit-outer-spin-button]:[appearance:none] [&::-webkit-inner-spin-button]:[appearance:none]"
                        placeholder="25000"
                        value={form.budgetAmount || ""}
                        onChange={e => {
                          const val = e.target.value;
                          update("budgetAmount", val === "" ? 0 : Math.max(0, Number(val)));
                        }}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      {/* Min */}
                      <div className="relative">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 flex items-center gap-1 text-gray-500 text-sm">
                          Min <Coins className="w-4 h-4" />
                        </span>

                        <input
                          type="number"
                          className="w-full pl-20 pr-4 py-3 border border-slate-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 [&::-webkit-outer-spin-button]:[appearance:none] [&::-webkit-inner-spin-button]:[appearance:none]"
                          value={form.budgetMin || ""}
                          onChange={e => {
                            const val = e.target.value;
                            update("budgetMin", val === "" ? 0 : Math.max(0, Number(val)));
                          }}
                          onWheel={e => e.currentTarget.blur()}
                          min="1"
                          step="any"
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          onChange={e => {
                            const val = e.target.value;
                            update("budgetMax", val === "" ? 0 : Math.max(0, Number(val)));
                          }}
                          onWheel={e => e.currentTarget.blur()}
                          min="1"
                          step="any"
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      ...
                      <button
                        ...
                        className={`p-3 rounded-xl border text-center transition-all ${form.duration === opt.value
                            ? "border-blue-500 bg-blue-50"
                            : "border-gray-200 hover:border-gray-300"
                          }`}
                      >
              ...
              <div className="space-y-5">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    {
                      label: "Price" , value: form.budgetType === "fixed"
                      ...
                      <div className="space-y-2">
                        {form.milestones.filter(m => m.title).map((m, i) => (
                          <div key={i} className="flex justify-between text-sm bg-gray-50 rounded-lg px-3 py-2">
                            ...
                            <span className="font-medium">{formatCurrency(m.amount, form.currency)}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {form.attachments.length > 0 && (
                    <div>
                      <span className="text-sm text-gray-500 block mb-2">Documents ({form.attachments.length})</span>
                      <div className="space-y-2">
                        {form.attachments.map((file, index) => (
                          <div key={`${file.url}-${index}`} className="flex justify-between text-sm bg-gray-50 rounded-lg px-3 py-2">
                            <span className="text-gray-800 truncate">{file.filename}</span>
                            {file.fileSize ? (
                              <span className="text-xs text-gray-500 ml-3">{(file.fileSize / 1024 / 1024).toFixed(2)} MB</span>
                            ) : null}
                          </div>
                        ))}
                      </div>
              ...
              {step < 4 ? (
                <div className="flex flex-col items-end gap-2">
                  <Button
                    ...
                    disabled={!canProceed() || isCompanyPostLockedNow}
                  ...
                  {(!canProceed() || isCompanyPostLockedNow) && (
                    <p className="text-xs text-red-500">
                      ...
                      {isCompanyPostLockedNow && ` Complete company profile first: ${companyProfileMissing.join(", ")}`}
                    </p>
                  )}
                </div>
              ) : (
                <Button
                  ...
                  disabled={submitting || isCompanyPostLockedNow}
